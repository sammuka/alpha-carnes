...
| 2 | Shell + DS fiel ao protótipo (layout, menu 9 grupos, tokens, componentes compartilhados) | 1 | aguardando_portao1 | [`2026-07-25-onda2-shell-ds.md`](../superpowers/plans/2026-07-25-onda2-shell-ds.md) | — | — | Plano tático escrito na branch `feature/onda2-plano-shell-ds`; **revisão 2 após Portão 1 `ajustar`** (veredito `4cbf6e9`, Monitor `3a1dbb62`): sha256 `b9839672…` (era `9fcd8c4c…`), 9 Tasks, 29 decisões fixadas e 34 invariantes no mapa DoD→teste. Os 10 achados foram fechados: critério sintático para o hex de seletor de atributo (`chart.tsx`, decisão 23), trechos das Tasks 3/5 recompilados sob `noUncheckedIndexedAccess`, teste de `AdminHeader` desambiguado, comando real de subida do backend (serviço `backend` do compose — não existe `start:dev`), rota de entrada `/` + destino pós-login pelo menu do perfil (decisão 26), efeitos do gate de grupo declarados (decisão 25), RBAC de UI de `/cadastros/rotas` e `/desossa/dashboard` com decisão numerada (24), `/admin/auditoria` (matriz linha 41) diferido para a Onda 3 com teste de guarda (27), CTA do login no token de ação (29) e as **7 dívidas herdadas da Onda 1 formalmente redirecionadas para a Onda 6** (decisão 28 + seção "Dívidas herdadas da Onda 1" — o plano da Onda 6 deve absorvê-las no mapa DoD→teste). Aguarda novo veredito do Portão 1. Implementação irá na branch `feature/onda2-shell-ds` |
