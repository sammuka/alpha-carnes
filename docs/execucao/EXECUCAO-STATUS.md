...
| 3 | Cadastros & Regras completos + Admin | 2 | planejando | just-in-time | — | — | Dependência Onda 2 `mergeada`. Plano tático em `feature/onda3-plano-cadastros-admin` (worktree `.worktrees/onda3-plan`). Deve absorver no mapa DoD→teste: **26 perdas** (decisão 25 Onda 2), conciliação catálogo×matriz (decisão 31), linha 41 matriz (decisão 27). P13 fechada por AD-04 (`ESTOQUE_*`) |
