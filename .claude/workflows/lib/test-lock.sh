...
for index in 1 2 3 4 5 6 7 8; do
  (
    set +e
    output="$("$lock_script" acquire "$stress_lock" "owner-$index" 0 2>&1)"
    code=$?
    set -e
    printf '%s:%s:%s\n' "$index" "$code" "$output" >> "$test_root/results"
    if [ "$code" -eq 0 ]; then
      sleep 1
      "$lock_script" release "$stress_lock" "owner-$index" >> "$test_root/releases"
    fi
  ) &
done
wait
