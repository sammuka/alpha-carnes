...
export function PesagemDestinacaoClient({ permissoes }: { permissoes: string[] }) {
  ...
  const podeEncerrarPesagem = permissoes.includes('CONFERENCIA_CONCLUIR');
  ...
  const carregarRecebimentos = useCallback(async () => {
    ...
    const ativos = pag.data.filter((r) => r.status === 'pesagem_em_andamento');
  ...
  const carregarFaltas = useCallback(async () => {
    const qs = recebimentoId ? `?recebimentoId=${encodeURIComponent(recebimentoId)}` : '';
    const res = await fetch(`/api/desossa/faltas${qs}`, { cache: 'no-store' });
    if (!res.ok) {
      setFaltas([]);
      return;
    }
  ...
  const buscaPedidoConcluido = useMemo(() => {
    const q = buscaPedido.trim().toLowerCase();
    if (!q || compativeisFiltrados.length > 0) return false;
    return (sugestao?.concluidos ?? []).some((nome) => nome.toLowerCase().includes(q));
  }, [buscaPedido, compativeisFiltrados.length, sugestao]);
  ...
  const encerrarPesagem = async () => {
    if (!recebimentoId || detalhe?.status !== 'pesagem_em_andamento' || !podeEncerrarPesagem) return;
    if (
      !window.confirm(
        'Encerrar a pesagem deste lote? O status passará a pesagem encerrada.',
      )
    ) {
      return;
    }
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`/api/operacao/recebimentos/${recebimentoId}/concluir-pesagem`, {
        method: 'POST',
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        setErro(extrairMensagemErro(data, 'Não foi possível encerrar a pesagem'));
        return;
      }
      setPeca(null);
      setSugestao(null);
      ...
      setErro('Erro de conexão ao encerrar a pesagem');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const podeEncerrarPesagemAtivo =
    podeEncerrarPesagem && Boolean(recebimentoId) && detalhe?.status === 'pesagem_em_andamento';

  const loteProntoParaCaptura = Boolean(recebimentoId && produtoBaseId);
    ...
    <div className="space-y-3">
      ...
      <div className="grid items-start gap-2.5 xl:grid-cols-[340px_1fr]">
          ...
          <CardContent className="space-y-2.5">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {podeManual && (
                  <Button
                    ...
                    disabled={!loteProntoParaCaptura || submitting}
                  >
                    Digitar
                  </Button>
                )}
            ...
            {destinoLocal && (
              <Button
                variant="destructive"
                className="w-full"
                data-testid="btn-mudar-destino"
                onClick={() => setDestinoLocal(null)}
                disabled={submitting}
              >
                Mudar destino
              </Button>
        ...
        <Card>
          ...
          <CardContent className="space-y-2.5">
            ...
            {sugestao && compativeisFiltrados.length === 0 && (
              <EmptyState
                title={buscaPedidoConcluido ? 'Pedido concluído' : 'Nenhum pedido de venda encontrado.'}
              />
            )}
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {podeEncerrarPesagemAtivo && (
            <Button
              variant="destructive"
              size="lg"
              className="h-auto min-w-0 flex-col items-start justify-center gap-0.5 py-2.5 text-left"
              data-testid="btn-encerrar-pesagem"
              onClick={() => void encerrarPesagem()}
              disabled={submitting}
            >
              <span className="text-[13px] font-semibold">Encerrar pesagem</span>
              <span className="text-[11px] font-normal text-white/80">
                Encerra a pesagem deste lote
              </span>
            </Button>
          )}
          {podeAssociar && (
