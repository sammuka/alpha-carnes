...
describe('Onda 9 — proveniência da migration de carga (frota + divergência)', () => {
  it('0025_onda9_carga_expand é DDL puro, sem DML', () => {
    const expand = readMigration('0025_onda9_carga_expand');
  ...
  it('journal contíguo: 0023 (Onda 7) → 0024 (Onda 8) → 0025 (Onda 9), sem gap', () => {
    const journal = readJson<Journal>(path.join(META_DIR, '_journal.json'));
    const entries = journal.entries.filter((entry) => entry.idx >= 23 && entry.idx <= 25);

    expect(journal.version).toBe('7');
    expect(journal.dialect).toBe('postgresql');
    expect(entries.map(({ idx, tag }) => ({ idx, tag }))).toEqual([
      { idx: 23, tag: '0023_onda7_desossa_expand' },
      { idx: 24, tag: '0024_onda8_estoque_expand' },
      { idx: 25, tag: '0025_onda9_carga_expand' },
    ]);
    expect(entries.every((entry) =>
      entry.version === '7' && entry.breakpoints,
    )).toBe(true);

    const snapshots = [23, 24, 25].map((idx) =>
      readJson<Snapshot>(
    ...
    const actualO9 = fs.readdirSync(MIGRATIONS_DIR)
      .filter((name) => /^0025_/.test(name));
    expect(new Set(actualO9)).toEqual(new Set(['0025_onda9_carga_expand.sql']));
  });
});
